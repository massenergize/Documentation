...
Below are a few components that have been created to suit MassEnergize's unique needs. All these components are made with react and use props. The current props of all these components are subject to change! Work on anything or change any thing to suit your situation as the occassion demands. However, take a look at the various properties before you start editing any components, your needs might already have been implemented! Let's get started :fire:
...
**`variation`** | Switch between three different button designs | `String` | `accent`, `union`, `normal` _(default = "normal")_
...
**`value` or `defaultValue`** | Any of the fields will work. Used to set the default value of the input _not textarea_ <span style="color:red">_(should not be null or undefined)_</span>|`String` | `"Any kind of text"`
...
| **`showOverlay`**               | Show a transaparent overlay over screen when in cropping mode                                                                                                       | `boolean`  | `true or false` _default = true_                            |
...
### <a name="me-section">MESECTIONWRAPPER</a>

Just small custom accordion that helps group related items. <br/>

| Property                 | Description                                                                 | Type      | Values Or Examples                     |
| ------------------------ | --------------------------------------------------------------------------- | --------- | -------------------------------------- |
| **`style`**              | Normal inline react css handling.                                           | `Object`  | Eg. `{{ height:30}}`                   |
| **`className`**          | Predefined css classes                                                      | `String`  | "som-random-css-class and-another-one" |
| **`collapsed`**          | Whether or not the content div should be collapsed                          | `boolean` | `true of false` _default = false_      |
...
### <a name="me-form-generator">MEFORMGENERATOR</a>

MECustom component that generates a form based on an array of **jsons**
...
| **`fields(required)`**    | An array of jsons                                                                                                     | `Array`    | Details below...                                                    |
| **`actionText`**          | Custom text to be displayed on the default submit button                                                              | `String`   | Eg. "Send", "Submit Text", "Submit Form" "Go", "Any Text" etc       |
...
```javascript
  ...
  return <FormGenerator 
          actionText="Save My Story" // appears on the submit button
           fields ={fields} 
           onSubmit ={(e,data,reset)=>{
             e.preventDefault(); 
             console.log(data); 
            reset();
            }} 
